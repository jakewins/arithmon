...
- All three map JSONs verified against story spec: downstairs 11x12 with
  4 tilesets at firstgids 1/2971/6835/10699, slug `rival_downstairs`, all
  8 collision rects present and pixel-exact; bedroom 7x7 with embedded
  `Interiors_16x16` at firstgid 9279 (tilecount 2592, columns 16),
  slug `rival_bedroom`, all 5 collision rects; office 7x7 with embedded
  `Office_interiors_shadowless_16x16` at firstgid 9279 (tilecount 528,
  columns 22), slug `rival_office`, all 3 collision rects. All maps have
  4 tile layers (Tile Layer 1/2/3 + Above player all opacity 1).
- All 3 event YAMLs `diff`-clean against upstream (0 bytes differ).
  Event counts: downstairs 10, bedroom 7, office 3 — matches story spec.
- `fashionista.png` byte-identical to upstream via `cmp`.
- All dialog msgids in `en_US.po` have non-empty `msgstr` (35 strings
  audited including both flashback sets, all interactable dialogs,
  and radio set).
- `src/game/data/maps.ts` correctly adds `CORE_INDOOR_STAIRS` to
  downstairs and `INTERIORS_16X16`/`OFFICE_INTERIORS` to bedroom/office;
  all referenced tileset constants were already defined.
- Pre-commit gates: `format:check`, `lint`, `tsc --noEmit`, and
  `npm test` (465 tests) all pass.
- `qa/paper-rival-test.ts` all 10 sub-tests pass (entry, stairwells,
  interactables, TV cutscene, package-daycare flashback round-trip,
  collision sanity).
- Visual comparison: live screenshots match the three committed upstream
  reference PNGs — all furniture/tiles in correct positions.
